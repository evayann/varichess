import { ByColor, Color, COLORS, Square, SquareSet } from "chessops";
import { ByRole, Role, ROLES, Piece } from './variantType';
...
export class Board implements Iterable<[Square, Piece]>, ByRole<SquareSet>, ByColor<SquareSet> {
    ...
    static zoo(): Board {
        const board = new Board();
        board.occupied = new SquareSet(0xffff, 0xffff_0000);
        ...
        board.bishop = SquareSet.empty();
        board.rook = SquareSet.empty();
        board.queen = SquareSet.empty();
        board.king = SquareSet.empty();

        board.unicorn = new SquareSet(0x42, 0x4200_0000);
        board.elephant = new SquareSet(0x24, 0x2400_0000);;
        board.corporal = new SquareSet(0xff00, 0x00ff_0000);
        board.giraffe = new SquareSet(0x81, 0x8100_0000);
        board.zebra = new SquareSet(0x8, 0x0800_0000);
        board.royalknight = new SquareSet(0x10, 0x1000_0000);
        return board;
    }
    ...
    reset(): void {
        ...
        this.unicorn = SquareSet.empty();
        this.elephant = SquareSet.empty();
        this.royalknight = SquareSet.empty();
    }
